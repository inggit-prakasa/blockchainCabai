...
function userFormSubmit() {
    ...
    if ($("form#userForm").parsley().isValid()) {
        ...
        var userAlamat = $("#userAlamat").val();
        var userRoles = $("#userRoles").val();
        var isActive = $("#isActive").is(":checked");
        var userImageAddress = $("#userProfileHash").val();

        globUserContract.methods.updateUserForAdmin(userWalletAddress, userName, userContactNo, userAlamat, userRoles, isActive, userImageAddress)
...
function addCultivationBatch() {
    ...
    if (batchFormInstance.validate()) {
        var sentraProduksi = $("#sentraProduksi").val().trim();
        var jenisCabai = $("#jenisCabai").val().trim();

        globMainContract.methods.addBasicDetails(sentraProduksi, jenisCabai)
            .send({
                from: globCoinbase,
            ...
            .on('receipt', function(receipt) {
                receiptMessage = "Transaksi Sukses";
                handleTransactionReceipt(receipt, receiptMessage);
                $("#batchFormModel").modal('hide');
                getCultivationEvents(globMainContract);
            })
